...
public class CnmToGranuleHandlerTest
	...
	/**
	 * Test that PerformFunction functions as expected when provided a CNM with filegroups AND files
	 *
	 * The result should be the same as testPerformFunctionFilegroups
	 */
	public void testPerformFunctionFilegroupsAndFiles() throws Exception {
		ClassLoader classLoader = getClass().getClassLoader();
		File inputJsonFile = new File(classLoader.getResource("inputFilesAndFilegroups.json").getFile());
		File expectedJsonFile = new File(classLoader.getResource("expectedFilegroups.json").getFile());
